...
plot_lake_data <- function(model_df){
  cols <- RColorBrewer::brewer.pal(8, "Dark2") # Set custom color palette for our plot - ooh we are fancy!! :-)
  
  ggplot() +
    geom_point(data = model_df, aes(x = datetime, y = air_temperature, color = "Air temperature")) +
    geom_point(data = model_df, aes(x = datetime, y = temperature, color = "Water temperature")) +
...
plot_airtemp_forecast <- function(lake_obs, noaa_past_selected, forecast_start_date){
  l.cols <- RColorBrewer::brewer.pal(8, "Set2")[-c(1, 2)] # Defining another custom color palette :-)
  
  ggplot() +
    geom_point(data = lake_obs, aes(x = as_date(datetime), y = air_temperature, color = "Observed air temp.")) +
    geom_line(data = noaa_past_selected, aes(x = as_date(datetime), y = air_temperature, group = parameter, color = "Forecasted air temp."), alpha = 0.6)+
